...
export default function Header({ initialQuery, showSearch = false }) {
  ...
  useEffect(() => {
    ...
    const fetchPendientes = async () => {
      if (!user || Number(user.idRol) !== 3) return setPendingServicios(0);
      try {
    ...
    return () => { window.removeEventListener('cart:updated', handleCartUpdate); mounted = false; clearInterval(intId); window.removeEventListener('focus', onFocus); window.removeEventListener('servicios:updated', onServiciosUpdated); };
  }, [user]);
          ...
          <Box sx={{
            ...
            <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
              {user && Number(user.idRol) === 3 && (
                <>
                  <Button component={RouterLink} to="/servicios-admin" variant="text" size="small" startIcon={pendingServicios > 0 ? <Badge badgeContent={pendingServicios} color="error" sx={{ mr: 1 }} /> : null}>
                    Servicios
                  </Button>
